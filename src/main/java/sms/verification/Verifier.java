...
public class Verifier {
	...
	private CounterExample getInputsOnlyFromModel(Model model, Context ctx) throws VerificationException {
		int[] inputs = new int[verVarNames.length];
		//get the inputs from the model
		for (int i = 0; i < verVarNames.length; i++) {
			Expr<?> expr = ctx.mkIntConst(verVarNames[i]);
		...
		return new CounterExample(inputs, -1);
	}
	...
	public boolean verifyNeedsRefinement(String program, String programTwo) throws Exception {
		//String query = VerificationQueries.generateProgramCanSatisfyQuery(program, logic, functionName, functionCallString, 
		...
		String query = 
		VerificationQueries.generateNeedsRefinementQuery
		(		program, programTwo, logic, functionName, functionCallString, 
				functionDeclarationString, assertionString, null, definedFunctions, verVarNames, synthesisVariableNames, eqInvocations, distInvocations,
	...
	public boolean verifyInterchangeableConfiguration(String program, String[] partials, String teString) throws Exception {
		//String query = VerificationQueries.generateProgramCanSatisfyQuery(program, logic, functionName, functionCallString, 
			//	functionDeclarationString, assertionString, partials, definedFunctions, verVarNames, synthesisVariableNames, invocations, programsByInvocation,
				//distinctInvocation, diProgram);
		...
		String query = VerificationQueries.generateInterchangeableConstructionQuery(
				...
				previousConfigurations, teString);
		//System.out.println("Inter Config for program " + program +"\n" + query);
		try(Context ctx = new Context()) {
...
public boolean verifyDistinctConfigurationCanSatisfy(String program,
	 ...
	 * @return True if partials are verified to cover the input space, false otherwise
	 */
	public boolean verifyInductiveProgramSound(String programToCheck, ArrayList<String> mappings) {
		...
		String query = VerificationQueries.generateInductiveProgramSoundQuery(programToCheck, mappings,
				logic, functionName, functionCallString, functionDeclarationString,
				assertionString, definedFunctions, verVarNames,  synthesisVariableNames);
		
		//System.out.println(query);
		/*
			String query = VerificationQueries.generateProgramVerificationQuery
					(null, null, logic, functionName, functionCallString, functionDeclarationString,
							assertionString, null, null, null, definedFunctions, verVarNames, synthesisVariableNames, true);*/
		//	////System.out.println(query);
	...
	public static boolean verifyEditedAssertionImpliesOriginal(String editedAssertionString, Benchmark benchmark) {
	...
	String query = VerificationQueries.generateEditImpliesOriginalQuery(editedAssertionString, benchmark.getAssertionString(), benchmark.getFunctionName(),
			benchmark.getDefinedFunctions(), benchmark.getVariableNames());
	System.out.println(query);
		//open context with try with resources, ensuring it will be closed after try block
		try(Context ctx = new Context()) {
	...
	public static boolean verifyEditedEquivalentToOriginal(String originalAssertionString, String editedAssertionString, Benchmark benchmark ) {
	...
	String query = VerificationQueries.generateEditedEquivalentToOriginalQuery(originalAssertionString, editedAssertionString, benchmark.getFunctionName(),
			benchmark.getDefinedFunctions(), benchmark.getVariableNames());
	//System.out.println(query);
		//open context with try with resources, ensuring it will be closed after try block
		try(Context ctx = new Context()) {
	...
	public static boolean verifySetsEquivalentForProp(String prop, ArrayList<String> partialsOne, ArrayList<String> partialsTwo, Benchmark benchmark ) {
	...
	//String query = VerificationQueries.generateEditedEquivalentToOriginalQuery(originalAssertionString, editedAssertionString, benchmark.getFunctionName(),
		...
		/*
		 * 
		 * (ArrayList<String> partialsOne, ArrayList<String> partialsTwo, String assertionString, 
		...
		String query =  VerificationQueries.generateSetsEquivalentQuery(partialsOne, partialsTwo, prop,
				benchmark.getLogic(), 
				benchmark.getFunctionName(), verifier.getFunctionCallString(), 
				verifier.getFunctionDeclarationString(), verifier.getDefinedFunctions(), benchmark.getVariableNames());
	//System.out.println(query);
		//open context with try with resources, ensuring it will be closed after try block
		try(Context ctx = new Context()) {
	...
	public VerificationResult verifyMIPartialsWithResult(ArrayList<String> partials, ArrayList<String> possiblePrograms,
			ArrayList<InvocationsConfiguration> configurations) {
			...
			try(Context ctx = new Context()) {
				...
				if (status == Status.UNSATISFIABLE) {
					...
					return new VerificationResult(getInputsOnlyFromModel(solver.getModel(), ctx), Status.SATISFIABLE);
				} else {
					return new VerificationResult(null, Status.UNKNOWN);		
				}
			} catch (Exception e) {
			//	////System.out.println(e.getMessage());
	...
	public boolean verifyProgramFinal(String program) {
