...
import sms.multipleInvocation.InvocationsConfiguration;
...
public class VerificationQueries {
	...
	public static String generateInductiveProgramSoundQuery(String program, ArrayList<String> mappings,
			String logic, String functionName, String functionCallString, String functionDeclarationString, String assertionString,
		...
		String mappingsString = "(or false \n";
		
		for (int i = 0; i < mappings.size(); i++) {
			mappingsString += mappings.get(i) + "\n ";
		}
		mappingsString += ")";
		...
		if (program != null) {
			...
			retVal += "(assert (or ";
			retVal += "(and " + mappingsString + " " + "(not " +assertionString.replace(parPattern, parPattern+"_synthed") + "))\n";
			retVal += "(and (not " + mappingsString + ") " + assertionString.replace(parPattern, parPattern+"_synthed") + ")\n";
			retVal += "))";
			
		//	retVal += "(assert (not " + assertionString.replace(parPattern, parPattern+"_synthed") + "))\n";
		}
		
	//	retVal += "(assert " + mappingsString + ")\n";
	...
	public static String generateEditImpliesOriginalQuery(String editedAssertionString, String originalAssertionString,
			String functionName, String[] definedFunctions, String[] variableNames) {

		String retVal = "";
		retVal += "(set-logic LIA)\n";
		...
		//Declare function 
		retVal += "(declare-fun " + functionName + " (";
		...
		//Find a function that is correct in the edit and incorrect in the original.
		//If unsat means that a function that satisfies the edit will also satisfy the original.
		retVal += "(assert " + editedAssertionString + ")\n";
		retVal += "(assert (not " + originalAssertionString + "))\n";
	...
	public static String generateEditedEquivalentToOriginalQuery(String originalAssertionString, String editedAssertionString,
			String functionName, String[] definedFunctions, String[] variableNames) {
		...
		//Declare variables and build replacement token definition.
		String replacementDefinition = "(define-fun replacement_token (";
		
		for (int i = 0; i < variableNames.length; i++) {
			retVal += "(declare-fun " + variableNames[i] + " () Int)\n";
			replacementDefinition += "(" + variableNames[i] + " Int) ";
		}
		
		replacementDefinition += ") Bool false)";
		
		
		retVal += replacementDefinition + "\n";
		...
		retVal += "(assert (or \n";
		retVal += "(not (=> \n";
		retVal += editedAssertionString + "\n";
		retVal += originalAssertionString + "\n";
		retVal += "))\n";
	...
	public static String generateInterchangeableConstructionQuery(String program, 
			...
			ArrayList<ArrayList<String>> distInvocations, ArrayList<InvocationsConfiguration> previousConfigurations, String teString) {
		String defPattern = "define-fun " + functionName;
		String parPattern = "(" + functionName;
		String retVal = "";
		...
		retVal += functionDeclarationString.replace(defPattern, defPattern + "_" + 0).replace("funToken;",
				program) + "\n";
		retVal += "(assert (not " + assertionString.replace(parPattern, parPattern + "_" + 0) + "))\n";
		...
		//retVal += "(assert (= " + functionCallString + " " + program + "))\n";
		retVal += "(assert (= " + functionCallString + " " + functionCallString.replace(parPattern, parPattern + "_0") + "))\n";
		
		
		//System.out.println("functionCallString is " + functionCallString);
		...
		if (eqInvocations != null) {
			for (int i = 0; i < eqInvocations.size(); i++) {
				...
				//retVal += "(assert (= " + invocationString + " " + program + "))\n";
				retVal += "(assert (= " + invocationString + " " + functionCallString + "))\n";
				//System.out.println("Invocation String is " + invocationString);
		...
		if (distInvocations != null) {
			for (int i = 0; i < distInvocations.size(); i++) {
				...
				//retVal += "(assert (distinct " + invocationString + " " + program + "))\n";
				retVal += "(assert (distinct " + invocationString + " " + functionCallString + "))\n";
		//		System.out.println("Invocation String is " + invocationString);
				//String teString = "gte";
				if (i == distInvocations.size()-1) {
					
					if (teString.equals("lte")) {
						retVal += "(assert ";
						retVal += "(=> (<= " + functionCallString + " " + invocationString + ") ";
						retVal += "(= " + functionCallString + " " + invocationString + ")) ";
						retVal += ")\n";
					} else {
						retVal += "(assert ";
					...
					//Yeah here is where we force the >=/<= to imply 
					/*retVal += "(assert (and ";
					retVal += "(=> (<= " + invocationString + " " + functionCallString + ") ";
					retVal += "(= " + invocationString + " " + functionCallString + ")) ";
					retVal += "(=> (>= " + invocationString + " " + functionCallString + ") ";
					retVal += "(= " + invocationString + " " + functionCallString + ")) ";	
					retVal += "))\n";*/
		...
		for (int i = 0; i < previousConfigurations.size(); i++) {
			...
			for (int j = 0; j < previousDistConfiguration.size(); j++) {
				...
				//functionCallString.replace(parPattern, parPattern + "_config_" + i)
				//retVal += "(assert (distinct " + invocationString + " " + program + "))\n";
				retVal += "(assert (distinct " + invocationString + " " + functionCallString.replace(parPattern, parPattern + "_config_" + i) + "))\n";
			...
			for (int j = 0; j < previousEqConfiguration.size(); j++) {
				...
				retVal += "(assert (= " + invocationString + " " + functionCallString.replace(parPattern, parPattern + "_config_" + i) + "))\n";
	...
	public static String generateNeedsRefinementQuery(String program, String programTwo, 
			String logic, String functionName, String functionCallString, String functionDeclarationString, String assertionString,
			String[] foundPartials, String[] definedFunctions,
			String[] variableNames, String[] tempVarNames, ArrayList<ArrayList<String>> eqInvocations,
		...
		String decFunStringTwo = "(declare-fun " + functionName + "_check (";
		...
		retVal += decFunStringTwo;
		
		
		retVal += functionDeclarationString.replace(defPattern, defPattern + "_first").replace("funToken;",
				program) + "\n";
		retVal += "(assert (not " + assertionString.replace(parPattern, parPattern + "_first") + "))\n";
		
		
		retVal += functionDeclarationString.replace(defPattern, defPattern + "_second").replace("funToken;",
				programTwo) + "\n";
		
		//Assert that the target function is satisfiable
		retVal += "(assert (not (=> \n"
				+ assertionString + "\n" +
				assertionString.replace(parPattern, parPattern+"_check") +
				")))\n";
		...
		//retVal += "(assert (= " + functionCallString + " " + program + "))\n";
		retVal += "(assert (= " + functionCallString + " " + functionCallString.replace(parPattern, parPattern + "_first") + "))\n";
		retVal += "(assert (= " + functionCallString.replace(parPattern, parPattern+"_check") + " " + functionCallString.replace(parPattern, parPattern + "_second") + "))\n";
		...
		if (eqInvocations != null) {
			for (int i = 0; i < eqInvocations.size(); i++) {
				...
				//retVal += "(assert (= " + invocationString + " " + program + "))\n";
				retVal += "(assert (= " + invocationString + " " + functionCallString + "))\n";
				retVal += "(assert (= " + invocationString.replace(parPattern, parPattern+"_check") + " " + functionCallString.replace(parPattern, parPattern+"_check") + "))\n";
		...
		if (distInvocations != null) {
			for (int i = 0; i < distInvocations.size(); i++) {
				...
				//retVal += "(assert (distinct " + invocationString + " " + program + "))\n";
				retVal += "(assert (distinct " + invocationString + " " + functionCallString + "))\n";
				retVal += "(assert (distinct " + invocationString.replace(parPattern, parPattern+"_check") + " " + functionCallString.replace(parPattern, parPattern+"_check") + "))\n";
		...
		if (foundPartials != null) {
			...
			for (int i = 0; i < foundPartials.length; i++) {
				//if (!foundPartials[i].equals(program)) {
					retVal += functionDeclarationString.replace(defPattern, defPattern + "_fp_" + i).replace("funToken;",
							foundPartials[i]) + "\n";
					retVal += "(assert (not " + assertionString.replace(parPattern, parPattern + "_fp_" + i) + "))\n";
				//}
			}
	...
	public static String generateSetsEquivalentQuery(ArrayList<String> partialsOne, ArrayList<String> partialsTwo, String assertionString, 
			String logic, String functionName,
			String functionCallString, String functionDeclarationString,
			String[] definedFunctions, String[] variableNames) {
		String defPattern = "define-fun " + functionName;
		String parPattern = "(" + functionName;
		String retVal = "";
		...
		String partialsOneCheck = "(and true";
		for (int i = 0; i < partialsOne.size(); i++) {
			retVal += functionDeclarationString.replace(defPattern, defPattern + "_list_one_" + i).replace("funToken;",
					partialsOne.get(i)) + "\n";
			partialsOneCheck += " (not " + assertionString.replace(parPattern, parPattern + "_list_one_" + i) + ")\n";
		}
		
		partialsOneCheck += ")";
		
		String partialsTwoCheck = "(and true";
		for (int i = 0; i < partialsTwo.size(); i++) {
			retVal += functionDeclarationString.replace(defPattern, defPattern + "_list_two_" + i).replace("funToken;",
					partialsTwo.get(i)) + "\n";
			partialsTwoCheck += " (not " + assertionString.replace(parPattern, parPattern + "_list_two_" + i) + ")\n";
		}
		...
		retVal += "(assert (or \n";
		retVal += "(not (=> \n";
		retVal += partialsOneCheck + "\n";
		retVal += partialsTwoCheck + "\n";
		retVal += "))\n";
