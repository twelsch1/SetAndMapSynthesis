...
public class CVCPredicateSynthesizer extends Synthesizer {
	...
	private String buildCVCQuery(String targetProgram, Benchmark benchmark, String[] remainingPartials, String[] globalConstraints) {
		...
		if (remainingPartials != null && remainingPartials.length != 0) {
			...
			remPartialsAssertions = "(and true ";
			for (int i = 0; i < remainingPartials.length; i++) {
				...
				remPartialsAssertions +=  "(not " + assertionString.replace(parPattern, parPattern + "_remaining_" + i) + ") ";
			}
		...
		//System.out.println("Hello " + remPartialsDefinitions);
		//System.out.println("Hi " + remPartialsAssertions);
		...
		/*
		query += "(constraint (=> " + predicateFunctionInvocation + " (and " + benchmark.getAssertionString() + " " + remPartialsAssertions + ")))\n";
		
		
		query += "(constraint (=> (not " + predicateFunctionInvocation + ") " + "(not (and " + benchmark.getAssertionString() + " " + remPartialsAssertions
		+ "))))\n";*/
	...
	/*	query += "(constraint (=> " + gcs + " ";
		...
		*/
		...
		System.out.println(targetProgram);
		System.out.println(query);
		return query;
	}
	
	private static String buildCVCQuery(String targetProgram, Benchmark benchmark, String modifiedAssertionString) {
		...
		String funcName = benchmark.getFunctionName();
		String assertionString = modifiedAssertionString;
		String tmp = benchmark.getFunString();
		////System.out.println("FUN STRING " + tmp);
		tmp = tmp.substring(0, tmp.lastIndexOf(")"));
		tmp = tmp.substring(0, tmp.lastIndexOf(")"));
		...
		query += "(constraint (=> " + predicateFunctionInvocation + " " + assertionString + "))\n";
		
				
		query += "(constraint (=> (not " + predicateFunctionInvocation + ") " + "(not " + assertionString
			+ ")))\n";
	...
	public static String synthesizeCompleteMapping(String targetPartial, Benchmark benchmark, String modifiedAssertionString, String cvcLocation) {
		...
		try {
			...
			String query = buildCVCQuery(targetPartial, benchmark, modifiedAssertionString);
